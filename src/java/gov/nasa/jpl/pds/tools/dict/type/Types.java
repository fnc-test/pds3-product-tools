// Copyright 2006, by the California Institute of 
// Technology. ALL RIGHTS RESERVED. United States Government 
// Sponsorship acknowledged. Any commercial use must be negotiated with 
// the Office of Technology Transfer at the California Institute of 
// Technology.
//
// This software may be subject to U.S. export control laws. By 
// accepting this software, the user agrees to comply with all 
// applicable U.S. export laws and regulations. User has the 
// responsibility to obtain export licenses, or other export authority 
// as may be required before exporting such information to foreign 
// countries or providing access to foreign persons.
//
// $Id$ 
//

package gov.nasa.jpl.pds.tools.dict.type;

/**
 * @author pramirez
 * @version $Revision$
 * 
 */
public interface Types {
    public final static String INTEGER = "INTEGER";
    public final static String CHARACTER = "CHARACTER";
    public final static String REAL = "REAL";
    public final static String IDENTIFIER = "IDENTIFIER";
    public final static String TIME = "TIME";
    public final static String DATE = "DATE";
    public final static String CONTEXT_DEPENDENT = "CONTEXT_DEPENDENT";
    public final static String CONTEXTDEPENDENT = "CONTEXTDEPENDENT";
    public final static String NONDECIMAL = "NONDECIMAL";
    public final static String NON_DECIMAL = "NON_DECIMAL";
    public final static String DOUBLE = "DOUBLE";
    public final static String EXPONENTIAL = "EXPONENTIAL";
    public final static String BIBLIO = "BIBLIO";
    public final static String ASCII_INTEGER = "ASCII_INTEGER";
    public final static String ALPHABET = "ALPHABET";
    public final static String ALPHANUMERIC = "ALPHANUMERIC";
    public final static String DATA_SET = "DATA_SET";
}
