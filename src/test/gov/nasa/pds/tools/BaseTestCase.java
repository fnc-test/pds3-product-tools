...
public abstract class BaseTestCase extends TestCase {
    ...
    public void assertProblemEquals(final LabelParserException problem,
            ...
            Object... arguments) {
        assertEquals("Lines do not match: ", lineNumber, problem
                .getLineNumber());
    ...
    public Dictionary getDictionary() {
        if (dictionary == null) {
            try {
                dictionary = DictionaryParser.parse(new File(TEST_DIR,
                        "pdsdd.full"));
                final Dictionary testDictionary = DictionaryParser
                        .parse(new File(TEST_DIR, "testdd.full"));
                if (testDictionary.getProblems().size() > 0) {
                    throw new RuntimeException("Test dictionary had "
                            + testDictionary.getProblems().size()
                            + " problems.");
                }
                dictionary.merge(testDictionary);
                for (final SimpleDictionaryChange change : dictionary
                        .getMergeChanges()) {
