...
public class ObjectDefinitionTest extends TestCase {
    ...
    public void testObjectMethods() {
        ...
        required.add("TARGET_NAME");
        definition.setRequiredElements(required);
        assertTrue(definition.mustHaveElement("TARGET_NAME"));
        assertTrue(definition.canHaveElement("TARGET_NAME"));
        assertTrue(!definition.mustHaveElement("BLAH"));
        assertTrue(!definition.canHaveElement("BLAH"));
        assertTrue(definition.hasElement("TARGET_NAME"));
        assertTrue(!definition.hasElement("BLAH"));
        
        assertEquals(0, definition.getOptionalElements().size());
        List optional = new ArrayList();
        optional.add("MISSION_NAME");
        definition.setOptionalElements(optional);
        assertTrue(definition.canHaveElement("MISSION_NAME"));
        assertTrue(!definition.mustHaveElement("MISSION_NAME"));
        
        assertEquals(0, definition.getRequiredObjects().size());
        definition.setRequiredObjects(required);
        assertTrue(definition.mustHaveObject("TARGET_NAME"));
        assertTrue(definition.canHaveObject("TARGET_NAME"));
        assertTrue(!definition.mustHaveObject("BLAH"));
        assertTrue(!definition.canHaveObject("BLAH"));
        assertTrue(definition.hasObject("TARGET_NAME"));
        assertTrue(!definition.hasObject("BLAH"));
        
        assertEquals(0, definition.getOptionalObjects().size());
        definition.setOptionalObjects(optional);
        assertTrue(definition.canHaveObject("MISSION_NAME"));
        assertTrue(!definition.mustHaveObject("MISSION_NAME"));
    }
