...
public class ElementValidator {
  ...
  public static boolean validate(AttributeStatement attribute, Label label,
    ...
    if (attribute.hasNamespace()) {
      if (attribute.getNamespace().length() > NAMESPACE_LENGTH) {
        valid = false;
        label.addProblem(attribute, "parser.error.longNamespace", //$NON-NLS-1$
            ProblemType.EXCESSIVE_NAMESPACE_LENGTH, ELEMENT_IDENT_LENGTH);
      }
    }
    ...
    if (attribute.getElementIdentifier().length() > ELEMENT_IDENT_LENGTH) {
      valid = false;
      label.addProblem(attribute, "parser.error.longIdentifier", //$NON-NLS-1$
          ProblemType.EXCESSIVE_IDENTIFIER_LENGTH, ELEMENT_IDENT_LENGTH);
    }
  ...
  private static boolean validate(final AttributeStatement attribute,
    ...
    if (value == null) {
      ...
      for (Value v : ((Collection<Value>) value)) {
        validValues = validate(attribute, label, dictionary, definition,
            checker, v);
        // Don't want to set to true if has already been set to false
        if (!validValues)
          valid = false;
        ...
        if (!scalar.isSupportedPDSType(definition.getDataType())) {
          ...
          label
              .addProblem(
                  ...
                  scalar.getClass().getName().substring(
                      scalar.getClass().getName().lastIndexOf(".") + 1), scalar.toString()); //$NON-NLS-1$
        } else {
          ...
          if (definition.hasValidValues()) {
            ...
            if (normalizedValue.trim().equals("")) { //$NON-NLS-1$
              label.addProblem(attribute, "parser.error.missingValue", //$NON-NLS-1$
                  ProblemType.MISSING_VALUE, attribute.getIdentifier());
            } else if (!definition.getValues().contains(normalizedValue)) {
              // Check for value with "_" substitution " "
              if (definition.getValues().contains(
                  normalizedValue.toUpperCase().replace("_", " "))) {
                label.addProblem(attribute, "parser.warning.manipulatedValue", //$NON-NLS-1$
                    ProblemType.MANIPULATED_VALUE, // whitespace
                    normalizedValue, normalizedValue.toUpperCase().replace("_",
                        " "), attribute.getIdentifier());
              } else if (definition.getValues().contains(
                  normalizedValue.toUpperCase())) {
                label.addProblem(attribute, "parser.warning.manipulatedValue", //$NON-NLS-1$
                    ProblemType.MANIPULATED_VALUE, // whitespace
                    normalizedValue, normalizedValue.toUpperCase(), attribute
                        .getIdentifier());
              } else if (!VALUE_TYPE_STATIC.equals(definition.getValueType())) {
                ...
                label.addProblem(attribute,
                    ...
                    ProblemType.UNKNOWN_VALUE, StrUtils
                        .truncate(normalizedValue), attribute.getIdentifier(),
                    normalizedValue);
              } else {
                valid = false;
                label.addProblem(attribute,
          ...
          // Check min length
          try {
            checker.checkMinLength(value.toString(), definition, attribute);
          ...
          // Check max length
          try {
            checker.checkMaxLength(value.toString(), definition, attribute);
          } catch (InvalidLengthException ile) {
            valid = false;
            label.addProblem(attribute, ile);
          }
          ...
          // do further checking
          if (checker instanceof NumericTypeChecker && castedValue != null
            ...
            if (definition.hasMinimum()) {
              try {
                numericChecker.checkMinValue((Number) castedValue, definition,
                    attribute);
            ...
            if (definition.hasMaximum()) {
              try {
                numericChecker.checkMaxValue((Number) castedValue, definition,
                    attribute);
              } catch (OutOfRangeException oor) {
                valid = false;
                label.addProblem(attribute, oor);
            ...
            if (value instanceof Numeric) {
              ...
              if (number.getUnits() != null
                ...
                label.addProblem(attribute,
                    ...
                    ProblemType.UNKNOWN_VALUE_TYPE, attribute.getIdentifier(),
                    number.getUnits());
              }
            }
          }
   ...
   * @return flag indicating whether or not the statement was valid against the
   *         definition found
