      ...
      if (this.definitions.containsKey(key)) {
        ...
        if (overwrite instanceof ElementDefinition) {
          ...
          if (overwriteElement.hasValidValues()) {
            final Collection<String> newVals = overwriteElement.getValues();
            final Collection<String> oldVals = localElement.getValues();
  ...
  public void addDefinition(Definition definition) {
    ...
    if (!this.definitions.containsKey(id)) {
      ...
      for (Iterator<Alias> i = definition.getAliases().iterator(); i.hasNext();) {
        Alias alias = i.next();
        final DictIdentifier aliasId = new DictIdentifier(alias, definition
            .getClass());
